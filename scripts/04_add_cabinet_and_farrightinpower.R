...
#   dates. We work through those dates from earliest to latest; the
...
#
# DATA SOURCES USED:
#   - data/intermediate/ess_harmonized.rds   (our ESS data so far)
...
#   - data/raw/parlgov/view_cabinet.csv  (to get country_id per country)
# ============================================================

library(dplyr)    # for data manipulation (filter, mutate, join, etc.)
library(readr)    # for read_csv()
library(stringr)  # for reading/parsing the SPSS text files
...
# We link them using ParlGov's view_cabinet.csv, which contains:
#   - country_name_short (ISO-3, e.g. DEU)
#   - country_id (ParlGov numeric)
#
# IMPORTANT FIX:
# Your mapping table must include *all* countries in your sample.
# GR and HR were missing before, which made country_id NA and then
# those countries were dropped later during cabinet assignment.
...
# Now add country_id to the ESS data
ess <- ess %>%
...
#
# HOW THIS WORKS:
# We join every respondent to every cabinet cutoff rule for their country
# (many-to-many join). Then we keep only rules where the cutoff date is
# BEFORE the respondent's interview date. The rule with the LATEST cutoff
...
# These will be dropped silently by filter() below.
...
message("\nFar-right cabinet IDs (", nrow(fr_cabinets), " unique cabinets):")
print(sort(fr_cabinets$cabinetid))
